...
function getSingleMedia($model, $collection = 'profile_image', $skip = true) {
    if (!\Auth::check() && $skip) {
        return asset('images/user/user_new.png');
    }
    $media = null;
    if ($model !== null) {
    ...
    if (getFileExistsCheck($media)) {
        ...
        switch ($collection) {
            case 'image_icon':
                $media = asset('images/user/user_new.png');
                break;
            case 'profile_image':
                $media = asset('images/user/user_new.png');
                break;
...
function calculate_commission($total_amount = 0, $provider_commission = 0, $commission_type = 'percent', $type = '', $totalEarning = 0, $count = 0, $provider_id = null) {
    if ($total_amount === 0) {
        return [
            'value' => '-',
            'number_format' => 0
    ...
    // Use custom commission if provider_id is provided
    if ($provider_id) {
        $provider = \App\Models\User::find($provider_id);
        if ($provider) {
            $effectiveCommission = $provider->getEffectiveCommission();
            if ($effectiveCommission) {
                $provider_commission = $effectiveCommission['value'];
                $commission_type = $effectiveCommission['type'] === 'percentage' ? 'percent' : $effectiveCommission['type'];
            }
        }
    }
    ...
    foreach ($revenuedata as $key => $value) {
        ...
        if (!empty($booking)) {
            ...
            if ($effectiveCommission) {
                ...
                $provider_commission = optional($provider->providertype)->commission ?? 0;
                $provider_type = optional($provider->providertype)->type ?? 'fixed';
            }

            $earning = ($total_amount) - ($provider_commission);
            if ($provider_type === 'percent' || $provider_type === 'percentage') {
                $earning = ($total_amount) * $provider_commission / 100;
            }
