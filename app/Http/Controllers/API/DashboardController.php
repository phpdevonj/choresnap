...
class DashboardController extends Controller {
    ...
    public function dashboardDetail(Request $request) {
        ...
        $service = ServiceResource::collection($service->with(['providers', 'category', 'serviceRating', 'providerServiceAddress', 'media'])->orderBy('id', 'desc')->paginate($per_page));
    ...
    public function providerDashboard(Request $request) {
        ...
        $thisYear = request()->input('this_year') ?? Carbon::now()->year;
        $thisMonth = request()->input('this_month') ?? Carbon::now()->month;
        $todayDate = request()->input('this_month') ?? Carbon::now()->day;
...
//        $stripeDetails = \Illuminate\Support\Facades\Cache::remember('stripe_account_' . $provider->id, now()->addHours(2), function () use ($provider) {
//            return $this->getStripeAccountDetails($provider->stripe_account_id, $provider->country_id);
//        });
...
//            "terms_conditions" => route('user.term_conditions'),
            "terms_conditions" => 'https://choresnap.com/terms-and-conditions/',
//            "privacy_policy"=> $privacy_policy,
//            "privacy_policy" => route('user.privacy_policy'),
            "privacy_policy" => 'https://choresnap.com/privacy-policy/',
//            "cookie_policy" => route('user.cookie_policy'),
            "cookie_policy" => 'https://choresnap.com/cookie-policy/',
//            "payment_policy" => route('user.payment_policy'),
            "payment_policy" => 'https://choresnap.com/payment-policy/',
            "schedule_cancellation_policy" => 'https://choresnap.com/schedule-cancellation-policy/',
//            "help_support" => route('user.help_support'),
    ...
    public function sendFcm(Request $request, $id) {
        ...
        $user = User::select('fcm_token')->find($id);
        if (!$user) {
            return response()->json(['errors' => ['fcm_token' => 'No User found.']], 422);
        }

        if (!$user->fcm_token) {
            return response()->json(['errors' => ['fcm_token' => 'Token not found.']], 422);
        }

        GoogleFcm::sendNotification($user->fcm_token, $request->title, $request->body);
