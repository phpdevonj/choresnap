...
class BookingController extends Controller {
    ...
    public function getBookingList(Request $request) {
                ...
                $query->where('id', 'LIKE', "%$search%")
                    ...
                    ->orWhereHas('provider', function ($providerQuery) use ($search) {
                        $providerQuery->where(function ($nameQuery) use ($search) {
                            $nameQuery->where(function ($q) use ($search) {
        ...
        if ($request->has('per_page') && !empty($request->per_page)) {
            ...
            if ($request->per_page === 'all') {
                $per_page = min($booking->count(), 200); // capped limit
            }
        }
        $orderBy = 'desc';
        ...
        $bookingsPaginated = $booking->orderBy('updated_at', $orderBy)->paginate($per_page);
        $items = BookingResource::collection($bookingsPaginated);

        $response = [
            'pagination' => [
                'total_items' => $bookingsPaginated->total(),
                'per_page' => $bookingsPaginated->perPage(),
                'currentPage' => $bookingsPaginated->currentPage(),
                'totalPages' => $bookingsPaginated->lastPage(),
                'from' => $bookingsPaginated->firstItem(),
                'to' => $bookingsPaginated->lastItem(),
                'next_page' => $bookingsPaginated->nextPageUrl(),
                'previous_page' => $bookingsPaginated->previousPageUrl(),
            ],
            'data' => $items,
        ];
    ...
    public function bookingUpdate(Request $request) {
        ...
        if (($data['status'] == 'rejected' || $data['status'] == 'cancelled') && $data['payment_status'] == 'advanced_paid') {
            ...
            ServiceHelper::stripeRefund($bookingdata);
...
//            } elseif ($data['status'] == "completed" || $data['status'] == "pending_approval") {
            } elseif ($data['status'] == "completed") {
                //complete booking provider/user
                $activity_type = 'update_booking_status';
                $activity_data = [
                    'activity_type' => $activity_type,
                    'booking_id' => $id,
                ...
                $activity_type = 'cancel_booking_provider';
                $activity_data = [
                    'activity_type' => $activity_type,
                    'booking_id' => $id,
                    'booking' => $bookingdata,
    ...
    public function getHandymanRatingList(Request $request) {
        ...
        if ($request->has('per_page') && !empty($request->per_page)) {
            ...
            if ($request->per_page === 'all') {
                $per_page = min($handymanratings->count(), 200); // capped limit
            }
        }
    ...
    public function getUserRatings(Request $request) {
        ...
        if ($request->has('per_page') && !empty($request->per_page)) {
            ...
            if ($request->per_page === 'all') {
                $per_page = min($ratings->count(), 200); // capped limit
            }
        }
