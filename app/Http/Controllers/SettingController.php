...
class SettingController extends Controller {
    ...
    public function cookiePolicy(Request $request) {
        $setting_data = Setting::where('type', 'cookie_policy')->where('key', 'cookie_policy')->first();
        $pageTitle = __('messages.cookie_policy');
        $assets = ['textarea'];
        return view('setting.cookie_policy_form', compact('setting_data', 'pageTitle', 'assets'));
    }
    ...
    public function saveCookiePolicy(Request $request) {
        ...
        $result = Setting::updateOrCreate(['id' => $request->id], $setting_data);
        if ($result->wasRecentlyCreated) {
            $message = __('messages.save_form', ['form' => __('messages.cookie_policy')]);
        } else {
            $message = __('messages.update_form', ['form' => __('messages.cookie_policy')]);
        }

        return redirect()->route('cookie-policy')->withsuccess($message);
    }
    ...
    public function paymentPolicy(Request $request) {
        ...
        $pageTitle = __('messages.payment_policy');
        $assets = ['textarea'];
        return view('setting.payment_policy_form', compact('setting_data', 'pageTitle', 'assets'));
    }
    ...
    public function savePaymentPolicy(Request $request) {
        ...
        $result = Setting::updateOrCreate(['id' => $request->id], $setting_data);
        if ($result->wasRecentlyCreated) {
            $message = __('messages.save_form', ['form' => __('messages.payment_policy')]);
        } else {
            $message = __('messages.update_form', ['form' => __('messages.payment_policy')]);
        }
    ...
    public function updateCommission(Request $request, $id) {
        ...
        // Validate custom commission
        if (!empty($request->custom_commission_type) || !empty($request->custom_commission_value)) {
            $request->validate([
                'custom_commission_type' => 'required|in:percentage,fixed',
                'custom_commission_value' => 'required|numeric|min:0|max:999999'
            ]);
            
            $provider->update([
                'custom_commission_type' => $request->custom_commission_type,
                'custom_commission_value' => floatval($request->custom_commission_value)
            ]);
            
            $message = __('messages.update_form', ['form' => __('messages.custom_commission')]);
        } else {
            // Clear custom commission
            $provider->update([
                'custom_commission_type' => null,
                'custom_commission_value' => null
            ]);
            
            $message = __('messages.custom_commission_cleared');
        }
        
        return redirect()->route('provider.commission', $id)->withSuccess($message);
    }

    public function clearCommission($id) {
        if (demoUserPermission()) {
            return redirect()->back()->withErrors(trans('messages.demo_permission_denied'));
        }
