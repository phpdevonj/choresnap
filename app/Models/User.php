...
class User extends Authenticatable implements HasMedia {
    ...
    protected static function boot() {
        ...
        ResetPassword::toMailUsing(function ($notifiable, $token) {
            ...
            $link = route('password.reset', compact('token', 'email'));
            return (new MailMessage)
                ->subject('Reset your ChoreSnap password 🔑')
                ->view('emails.reset_email', compact('link')); // Optional: use a custom blade view
        });
    ...
    /**
     * Get the effective commission for this provider
     * Returns custom commission if set, otherwise returns default from provider type
     */
    public function getEffectiveCommission() {
        if ($this->custom_commission_type && $this->custom_commission_value) {
            return [
                'type' => $this->custom_commission_type,
                'value' => floatval($this->custom_commission_value),
                'is_custom' => true
            ];
        }
        ...
        if ($this->providertype) {
            return [
                ...
                'value' => floatval($this->providertype->commission),
                'is_custom' => false
            ];
        }
    ...
    /**
     * Calculate commission amount based on total amount
     */
    public function calculateCommission($totalAmount) {
        ...
        if ($commission['type'] === 'percentage') {
            return ($totalAmount * $commission['value']) / 100;
        }
