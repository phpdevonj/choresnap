<x-master-layout>
        ...
        <div class="card-body">
            <div class="row justify-content-between">
                ...
                <div class="d-flex justify-content-end">
                    <div class="datatable-filter ml-auto d-flex align-items-center">
                        <label class="m-0 mr-2">{{ __('messages.status') }}</label>
                        <select name="column_status" id="column_status" class="select2 form-control"
                                data-filter="select" style="width: auto;">
                            <option value="">{{ __('messages.all') }}</option>
                            @foreach($filter['statusList'] as $statusInfo)
                                <option value="{{ $statusInfo->value }}">
                                    {{ ucwords($statusInfo->label) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    @if(isset($filter['paymentStatusList']))
                        <div class="datatable-filter ml-2 d-flex align-items-center">
                            <label class="m-0 mr-2">{{ __('messages.payment_status') }}</label>
                            <select name="column_payment_status" id="column_payment_status" class="select2 form-control"
                                    data-filter="select" style="width: auto;">
                                <option value="">{{ __('messages.all') }}</option>
                                @foreach($filter['paymentStatusList'] as $paymentStatusInfo)
                                    <option value="{{ $paymentStatusInfo->payment_status }}">
                                        {{ ucwords(str_replace('_',' ', $paymentStatusInfo->payment_status)) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    <script>
        $('#column_status').val('{{ request()->input('filter.column_status') }}').trigger('change');
            ...
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                    ...
                    "data": function (d) {
                        ...
                        let column_status = $('#column_status').val() || urlParams.get('filter[column_status]');
                        d.filter = {
                            column_status: column_status,
                            column_payment_status: $('#column_payment_status').val(),
                        }
                    },
                },
                columns: [
